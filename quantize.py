...
from optimization.quantization import Quantizer
...
def main(config):
    ...
    # setup quantizer
    error_margin = config['optimization']['quantizer']['error_margin']
    bitwidth_decrement = eval(config['optimization']['quantizer']['decrement'])
    quantizer = Quantizer(model, config=config, 
                          val_data_loader=val_data_loader,
                          error_margin=error_margin,
                          decrement=bitwidth_decrement)
    
    # quantize model
    ddict = quantizer.QuantizeNet2DynamicFixedPoint()
    
    # write results to file
    fpath = os.path.join(str(config.cfg_fname.parent), 'quantization.yml')
    index, i = '', 0
